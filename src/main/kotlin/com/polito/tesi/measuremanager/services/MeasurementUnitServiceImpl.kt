...
class MeasurementUnitServiceImpl(
    ...
    override fun get(id: Long): MeasurementUnitDTO? {
        val mu =
            if (isAdmin()) {
                ...
                mur.findByIdAndUser_UserId(id, getCurrentUserId())
            }
    ...
    override fun getByNodeId(nodeId: Long): List<MeasurementUnitDTO>  {
        ...
        if (n.user.userId != getCurrentUserId() && !isAdmin()) {
            throw OperationNotAllowed(
                "You can't get a MeasurementUnit owned by someone else",
            )
        }
    ...
    override fun delete(id: Long) {
        val mu = mur.findById(id).get()
        if (mu.user.userId != getCurrentUserId() && !isAdmin()) {
            throw OperationNotAllowed(
    ...
    override fun registerMu(
        ...
        if (existingMu != null) {
            ...
            val targetNode =
                nr.findByControlUnits_NetworkId(cuNetworkId)
